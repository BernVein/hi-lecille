...
export const PhotostripPreview: React.FC<PhotostripPreviewProps> = ({
  ...
  const slotCount = layout === "2-cut" || layout === "split-duo" ? 2 : 4;
  ...
  return (
    <div className="w-full bg-zinc-950/90 border border-zinc-800 rounded-2xl p-4 shadow-xl flex flex-col items-center">
      ...
      <div className="w-full flex justify-center py-2">
        <div
          className="shadow-2xl rounded-sm transition-all duration-300 relative border overflow-hidden p-3.5 sm:p-4 max-w-[280px] sm:max-w-[300px] w-full"
          style={{
        ...
        >
          ...
          <div className="text-center space-y-1 pt-1 border-t border-black/5">
            {couple.loveNote && (
            ...
            {selectedStickers.length > 0 && (
              <div className="text-xs tracking-wider pt-0.5">
                {selectedStickers.join(" ")}
              </div>
            )}
            <div className="text-[8px] font-mono opacity-60 pt-1 tracking-widest uppercase">
              PHOTOBOOTH • {filterConfig.name}
            </div>
          </div>
        </div>
      </div>
