...
export function usePeerSession({
  ...
  const bindStreamToVideo = useCallback((stream: MediaStream) => {
    ...
    const tryPlay = () => {
      video
        ...
        .catch((err) => {
          console.warn(
            "[WebRTC] Video play() failed, retrying in 500ms:",
            err.message,
          );
  ...
  const monitorConnection = useCallback((call: MediaConnection) => {
    ...
    if (!pc) {
      console.warn(
        "[WebRTC] Cannot access RTCPeerConnection for ICE monitoring",
      );
      return;
    }
      ...
      if (pc.iceConnectionState === "failed") {
        ...
        retryTimeoutRef.current = window.setTimeout(() => {
          if (partnerPeerIdRef.current) {
            console.log(
              "[WebRTC] Auto-retrying media call after ICE failure...",
            );
  ...
  const initiateMediaCall = useCallback(
    (targetPeerId: string) => {
      ...
      if (streamToSend && streamToSend.getVideoTracks().length > 0) {
        ...
        console.log(
          "[WebRTC] Calling with local video stream:",
          outgoingStream.getVideoTracks().length,
          "tracks",
        );
      } else {
        // Create a canvas-based "placeholder" stream so the call goes through
        console.log(
          "[WebRTC] No local video yet, calling with placeholder canvas stream",
        );
  ...
  const replaceTracksInCall = useCallback(
    (newStream: MediaStream) => {
      if (!mediaConnRef.current) return;
      const pc = (mediaConnRef.current as any).peerConnection as
        | RTCPeerConnection
        | undefined;
      if (!pc) return;
  ...
  useEffect(() => {
    ...
    if (mediaConnRef.current && hasReceivedStreamRef.current) {
      ...
      console.log(
        "[WebRTC] Local stream ready, calling partner:",
  ...
  const retryMediaConnection = useCallback(() => {
    if (partnerPeerIdRef.current) {
      console.log(
        "[WebRTC] Manual retry requested for:",
        partnerPeerIdRef.current,
      );
      hasReceivedStreamRef.current = false;
      initiateMediaCall(partnerPeerIdRef.current);
